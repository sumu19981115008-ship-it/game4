class_name LittlerootTown
extends Node2D

const TILE       := 16
const MAP_W      := 20
const MAP_H      := 20
const MAP_IMG    := "res://assets/maps/littleroot_town.png"
const MAP_JSON   := "res://assets/maps/littleroot_town.json"

# 出生点（搬家卡车旁，原版坐标 x=9,y=12）
const SPAWN_X    := 9
const SPAWN_Y    := 12
...
func _load_map_data() -> void:
	...
	if not file:
		push_error("LittlerootTown: 无法读取 " + MAP_JSON)
		return
...
func _build_background() -> void:
	...
	if not tex:
		push_error("LittlerootTown: 无法加载 " + MAP_IMG)
		return
...
func _build_collisions() -> void:
	if not _map_data.has("collision"):
		push_warning("LittlerootTown: 无碰撞数据，使用默认边框")
	...
	for row in MAP_H:
		...
		while col < MAP_W:
			if row < coll.size() and col < coll[row].size() and coll[row][col] == 1:
				...
				col += 1
	print("LittlerootTown: 碰撞体创建完成，共 %d 个" % wall_count)

func _add_wall(px: float, py: float, w: float, h: float) -> void:
	var body := StaticBody2D.new()
	body.collision_layer = 4   # CollisionLayers.WALL
...
func _setup_player() -> void:
	...
	# 优先使用场景切换时传入的目标出生点，否则使用本场景默认出生点
...
# ── 公共接口（与 ProceduralMap / TileMapZone 兼容）────────────────────────
