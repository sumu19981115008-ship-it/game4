class_name PetalburgCity
extends Node2D

const TILE       := 16
const MAP_W      := 30
const MAP_H      := 30
const MAP_IMG    := "res://assets/maps/petalburg_city.png"
const MAP_JSON   := "res://assets/maps/petalburg_city.json"
...
func _load_map_data() -> void:
	...
	if not file:
		push_error("PetalburgCity: 无法读取 " + MAP_JSON)
		return
...
func _build_background() -> void:
	...
	if not tex:
		push_error("PetalburgCity: 无法加载 " + MAP_IMG)
		return
...
func _build_collisions() -> void:
	if not _map_data.has("collision"):
		push_warning("PetalburgCity: 无碰撞数据，使用默认边框")
	...
	for row in MAP_H:
		...
		while col < MAP_W:
			if row < coll.size() and col < coll[row].size() and coll[row][col] == 1:
				...
				col += 1
	print("PetalburgCity: 碰撞体创建完成，共 %d 个" % wall_count)
