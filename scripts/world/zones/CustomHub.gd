class_name CustomHub
extends Node2D

const TILE    := 16
const MAP_W   := 60
const MAP_H   := 50
const MAP_IMG := "res://assets/maps/custom_hub.png"
const MAP_JSON:= "res://assets/maps/custom_hub.json"
...
func _load_map_data() -> void:
	...
	if not file:
		push_error("CustomHub: 无法读取 " + MAP_JSON)
		return
...
func _build_background() -> void:
	...
	if not tex:
		push_error("CustomHub: 无法加载 " + MAP_IMG)
		return
...
func _build_collisions() -> void:
	if not _map_data.has("collision"):
		push_warning("CustomHub: 无碰撞数据，使用默认边框")
	...
	for row in MAP_H:
		...
		while col < MAP_W:
			if row < coll.size() and col < coll[row].size() and coll[row][col] == 1:
				...
				col += 1
	print("CustomHub: 碰撞体 %d 个" % wall_count)
...
func get_spawn_position() -> Vector2:
	return Vector2((SPAWN_X + 0.5) * TILE, (SPAWN_Y + 0.5) * TILE)
