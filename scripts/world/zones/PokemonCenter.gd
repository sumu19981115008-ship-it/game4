class_name PokemonCenter
extends Node2D

const TILE       := 16
const MAP_W      := 14
const MAP_H      := 9
const MAP_IMG    := "res://assets/maps/pokemon_center_1f.png"
const MAP_JSON   := "res://assets/maps/pokemon_center_1f.json"
...
func _load_map_data() -> void:
	...
	if not file:
		push_error("PokemonCenter: 无法读取 " + MAP_JSON)
		return
...
func _build_background() -> void:
	...
	if not tex:
		push_error("PokemonCenter: 无法加载 " + MAP_IMG)
		return
...
func _build_collisions() -> void:
	if not _map_data.has("collision"):
		push_warning("PokemonCenter: 无碰撞数据，使用默认边框")
	...
	for row in MAP_H:
		...
		while col < MAP_W:
			if row < coll.size() and col < coll[row].size() and coll[row][col] == 1:
				...
				col += 1
	print("PokemonCenter: 碰撞体创建完成，共 %d 个" % wall_count)
