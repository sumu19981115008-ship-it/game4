#!/usr/bin/env python3
"""
把指定的 metatile 逐个渲染成一张大预览，方便 debug。
"""
import struct
import numpy as np
...
OUT       = GAME_ROOT / 'assets/maps/debug_pc_metas.png'
TILE_PX   = 8
META_PX   = 16

TARGET_METAS = [8,9,10,11,16,17,18,19,24,25,26,32,33,34,
                48,49,50,51,56,57,58,59,64,65,66,67,
                72,73,74,75,80,81,82,83,84,88,89,90,91,96,97,98,99]
...
def load_pals(d):
    return [parse_pal(d/'palettes'/f'{i:02d}.pal') if (d/'palettes'/f'{i:02d}.pal').exists()
...
def get_tile_rgba(arr,cols,idx,hflip,vflip,ps,combined,is_bottom=False):
    ...
    if ty+TILE_PX>arr.shape[0] or tx+TILE_PX>arr.shape[1]:
        return np.full((TILE_PX,TILE_PX,4),[255,0,255,255],dtype=np.uint8)
...
def render_meta(i,meta_raw,arr,cols,combined):
    ...
    for layer in range(2):
        for sub in range(4):
            ...
            if layer==0: canvas[py:py+TILE_PX,px:px+TILE_PX]=tile
            else:
                mask=tile[:,:,3]>0
                for ch in range(4): canvas[py:py+TILE_PX,px:px+TILE_PX,ch][mask]=tile[:,:,ch][mask]
    return canvas

SCALE=4
CELL=META_PX*SCALE+18

def main():
    meta_raw=CACHE.read_bytes()
    pals=load_pals(GEN_DIR)
    combined=[]
    for n in range(16): combined.extend(pals[n][:16])
    img_src=Image.open(GEN_DIR/'tiles.png')
    arr=np.array(img_src); cols=img_src.width//TILE_PX

    COLS=8
    rows=(len(TARGET_METAS)+COLS-1)//COLS
    out=Image.new('RGB',(COLS*CELL,rows*CELL),(20,20,20))
    draw=ImageDraw.Draw(out)

    for i,m in enumerate(TARGET_METAS):
        tile=render_meta(m,meta_raw,arr,cols,combined)
        bg=np.full((META_PX,META_PX,3),60,dtype=np.uint8)
        rgb=np.where(tile[:,:,3:4]>0,tile[:,:,:3],bg)
        t_img=Image.fromarray(rgb.astype(np.uint8),'RGB').resize((META_PX*SCALE,META_PX*SCALE),Image.NEAREST)
        cx=(i%COLS)*CELL+1; cy=(i//COLS)*CELL+1
        out.paste(t_img,(cx,cy))
        draw.text((cx,cy+META_PX*SCALE+1),str(m),fill=(220,220,80))
