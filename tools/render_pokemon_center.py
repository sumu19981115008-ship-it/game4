#!/usr/bin/env python3
"""
渲染 Pokemon Center 1F 内部地图为 PNG + JSON。
用法：python3 tools/render_pokemon_center.py
输出：assets/maps/pokemon_center_1f.png（224×144）
      assets/maps/pokemon_center_1f.json

tileset：primary_building + secondary_pokemon_center
调色板规则：combined[N]: N<6 → bld_pals[N], N>=6 → ctr_pals[N-6]
  有效调色板：bld[0]（灰白）、bld[1]（蓝白）、bld[5]（室内通用蓝调）
              ctr[6]（橙红台面）、ctr[7]（紫灰）、ctr[8]（暖白+金）、ctr[9]（亮黄）
  slot 6 = ctr[0]（全零，占位）→ 实际用 ctr[6-9]（slot 12-15 → ctr[6-9]）
"""
...
OUT_PNG   = GAME_ROOT / 'assets/maps/pokemon_center_1f.png'
OUT_JSON  = GAME_ROOT / 'assets/maps/pokemon_center_1f.json'
...
def make_combined(bld_pals, ctr_pals) -> list:
    """
    室内地图调色板布局（GBA 硬件绝对 slot 编号）：
      slot 0–5  → primary（building）：bld_pals[N]
      slot 6–15 → secondary（pokemon_center）：ctr_pals[N]（绝对索引，不减6）
    metatile 里的 pal_slot 字段就是硬件绝对编号，直接用。
    """
    combined = []
    for N in range(16):
        if N < 6:
            ...
            base_pal = ctr_pals[N]   # 绝对索引：slot6 → 06.pal，slot7 → 07.pal …
...
def render_metatile(global_idx, bld_meta_raw, ctr_meta_raw,
                    bld_arr, bld_cols, ctr_arr, ctr_cols, combined):
    is_sec = global_idx >= 512
    if is_sec:
        raw_data = ctr_meta_raw
        off = (global_idx - 512) * 16
    else:
        raw_data = bld_meta_raw
        off = global_idx * 16
    ...
    for layer in range(2):
        for sub in range(4):
            ...
            if tile_raw >= 512:
                ...
                tile_rgba = get_tile_rgba(ctr_arr, ctr_cols, tile_idx,
                                          hflip, vflip, pal_slot, combined,
                                          is_bottom=is_bottom)
            else:
                tile_rgba = get_tile_rgba(bld_arr, bld_cols, tile_raw,
                                          hflip, vflip, pal_slot, combined,
                                          is_bottom=is_bottom)
...
def main():
    print('下载地图数据...')
    bld_meta_raw = dl(f'{BASE_URL}/data/tilesets/primary/building/metatiles.bin')
    ctr_meta_raw = dl(f'{BASE_URL}/data/tilesets/secondary/pokemon_center/metatiles.bin')
    map_raw      = dl(f'{BASE_URL}/data/layouts/PokemonCenter_1F/map.bin')
    ...
    combined = make_combined(bld_pals, ctr_pals)
    ...
    for row in range(MAP_H):
        ...
        for col in range(MAP_W):
            ...
            tile = render_metatile(
                meta_idx, bld_meta_raw, ctr_meta_raw,
                bld_arr, bld_cols, ctr_arr, ctr_cols, combined
            )
            py = row * META_PX
            px = col * META_PX
    ...
    map_json = {
        ...
        'collision': collision,
        'transitions': [
            {'x': 7, 'y': 8, 'target': 'exit', 'spawn_x': 0, 'spawn_y': 0},
        ],
    }
