#!/usr/bin/env python3
"""
渲染 Route 101 地图为 PNG + JSON。
用法：python3 tools/render_route101.py
输出：assets/maps/route101.png（320×320）
      assets/maps/route101.json（碰撞数据）
"""
...
OUT_PNG   = GAME_ROOT / 'assets/maps/route101.png'
OUT_JSON  = GAME_ROOT / 'assets/maps/route101.json'
...
def main():
    ...
    map_raw      = dl(f'{BASE_URL}/data/layouts/Route101/map.bin')
    ...
    # Route101 只有草地/路径，无建筑，无需 override
    ...
    map_json = {
        ...
        'collision': collision,
        'transitions': [
            {'x': 10, 'y': 19, 'target': 'LittlerootTown', 'spawn_x': 9, 'spawn_y': 1},
            {'x': 10, 'y': 0,  'target': 'OldaleTown',     'spawn_x': 9, 'spawn_y': 18},
        ],
    }
