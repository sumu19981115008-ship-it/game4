#!/usr/bin/env python3
"""
渲染 Route 102 地图为 PNG + JSON。
用法：python3 tools/render_route102.py
输出：assets/maps/route102.png（800×320）
      assets/maps/route102.json（碰撞数据）
"""
...
OUT_PNG   = GAME_ROOT / 'assets/maps/route102.png'
OUT_JSON  = GAME_ROOT / 'assets/maps/route102.json'
...
def main():
    ...
    map_raw      = dl(f'{BASE_URL}/data/layouts/Route102/map.bin')
    ...
    # Route102 为草地/水路地形，无建筑，无需 override
    ...
    map_json = {
        ...
        'collision': collision,
        'transitions': [
            {'x': 0,       'y': 10, 'target': 'OldaleTown',    'spawn_x': 48, 'spawn_y': 10},
            {'x': MAP_W-1, 'y': 10, 'target': 'PetalburgCity', 'spawn_x': 1,  'spawn_y': 10},
        ],
    }
