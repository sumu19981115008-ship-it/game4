#!/usr/bin/env python3
"""
渲染 Oldale Town 地图为 PNG + JSON。
用法：python3 tools/render_oldale.py
输出：assets/maps/oldale_town.png（320×320）
      assets/maps/oldale_town.json（碰撞数据）

Override 分析（基于 scan_metatile_slots.py 结果）：
  metatile 620-639（pet，slot 10/9）：Oldale 建筑屋顶 + 水路装饰
    - slot 10 → pet[10]（米白/浅灰，Oldale 白色屋顶风格）
    - slot 9  → pet[9]（沙黄，屋顶装饰横梁）
  metatile 640-655（pet，混合 slot 1/2/9/10）：建筑主体 + 水路边缘
    - slot 10 → pet[10]（白色建筑体）
    - slot 9  → pet[9]（沙黄装饰）
"""
...
OUT_PNG   = GAME_ROOT / 'assets/maps/oldale_town.png'
OUT_JSON  = GAME_ROOT / 'assets/maps/oldale_town.json'
...
# Oldale 建筑屋顶（白色屋顶 + 沙黄横梁）
BLDG_ROOF_METAS = {620, 621, 622, 628, 629, 630, 636, 637, 638, 639}
# Oldale 建筑主体（白色墙体 + 沙黄装饰）
BLDG_BODY_METAS = {640, 641, 642, 643, 644, 645, 646, 647, 648, 649, 650, 651, 652, 653, 654, 655}
...
def main():
    ...
    map_raw      = dl(f'{BASE_URL}/data/layouts/OldaleTown/map.bin')
    ...
    # 建筑屋顶：slot10 → pet[10]（米白），slot9 → pet[9]（沙黄）
    combined_bldg_roof = make_combined(gen_pals, pet_pals, {9: 9, 10: 10})
    # 建筑主体：slot10 → pet[10]，slot9 → pet[9]
    combined_bldg_body = make_combined(gen_pals, pet_pals, {9: 9, 10: 10})
    ...
    for row in range(MAP_H):
        ...
        for col in range(MAP_W):
            ...
            if meta_idx in BLDG_ROOF_METAS:
                combined = combined_bldg_roof
            elif meta_idx in BLDG_BODY_METAS:
                combined = combined_bldg_body
            else:
    ...
    map_json = {
        ...
        'collision': collision,
        'transitions': [
            {'x': 9, 'y': 19, 'target': 'Route101', 'spawn_x': 10, 'spawn_y': 1},
        ],
    }
